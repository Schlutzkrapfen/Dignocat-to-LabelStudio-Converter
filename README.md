...
-- **`single`**: If there is a single label in the picture, combines all annotations into one and uses the farthest points of all labels (excluding those generated by the local AI) to try createing a bounding box over the whole denture.
...
If you want to split or refine a label into different parts, you can attach an AI model to it using the `ai:<model_name>` option.

1. **Model Location:** Save your model file inside the `AI-models/` directory (e.g., `AI-models/crown.pth`).
2. **Usage in Options:** Add `ai:` followed by the model filename in the `options` column in `label_mapping.csv` (e.g., `ai:crown.pth`).
...
Another option is to use `local_ai:<model_name>` to find new labels using a local YOLO AI model.

1. **Model Location:** Save your model file inside the `AI-models/` directory (e.g., `AI-models/caput.pt`).
2. **Usage in Options:** Add `local_ai:` followed by the model filename in the `options` column in `ai-label.csv` (e.g., `local_ai:caput.pt`).
3. **AI Mapping File:** Configure `ai-label.csv` to map the AI outputs.
   - Currently, the `options` column in `ai-label.csv` supports no options other than `local_ai:`.
   - Replace `diagnocat_label` with `ai_label` (the label name predicted by your AI model).
